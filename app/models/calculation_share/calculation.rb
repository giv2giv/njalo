    ...
    class << self
      ...
      def grant_step_1
        ...
        endowments.each do |endowment|
          ...
          donated_shares = endowment.donations.group(:user_id).sum(:shares_added)

          donated_shares.each do |user_id, shares_donor_donated|

            amount_per_charity = 0
            shares_per_charity = 0

            shares_donor_granted = endowment.grants.where("user_id = ? AND endowment_id = ? AND (status = ? OR status = ?)", user_id, endowment.id, "accepted", "pending_acceptance").sum(:shares_subtracted)
