class DonorCampaignsController < ApplicationController
  ...
  def index
    if params[:query].present?
      @charities = DonorCampaign.search(params[:query], page: params[:page], limit:25)
    else
      @charities = DonorCampaign.all.page params[:page]
    end
...
#    end
  ...
  def autocomplete
    render json: DonorCampaign.search(params[:query], fields: [{name: :text_start}], limit: 15).map(&:name)
  end
  ...
  def show
    ...
    donation = Donation.add_donation(
      donor_id: donor.id,
      donor_campaign_id: @campaign.id,
      transaction_id: transaction.id.to_s,
      gross_amount: gross_amount,
      transaction_fee: transaction_fee, net_amount
    )
...
=end
  ...
  def create
    @campaign = DonorCampaign.new(campaign_params)
    @campaign.save
    respond_with(@campaign)
  end
  ...
  private
    def set_campaign
      @campaign = DonorCampaign.where("(njalo_id=? OR slug=?)", params[:id], params[:id]).last
    end
